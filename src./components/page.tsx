...
//   addFavorite,
//   setFavorites,
// } from "@/store/slicers/weatherReducer";
  ...
  useEffect(() => {
    const fetchPopularCities = async () => {
      const responses = await Promise.all(
        POPULAR_CITIES.map((city) => dispatch(fetchWeather(city)))
      );
      setPopularCitiesWeather(
        responses.map((res) => res.payload).filter(Boolean)
      );
    };
    fetchPopularCities();
  }, [dispatch]);
    ...
    <div className="max-w-lg mx-auto mt-10 p-5 border rounded-lg shadow-lg">
      ...
      <div className="mb-4">
        ...
        <button
          ...
          className="w-full mt-2 p-2 bg-gray-300 text-black rounded cursor-pointer"
        >
          Пошук
        </button>
      </div>
